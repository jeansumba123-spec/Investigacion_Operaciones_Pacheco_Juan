import streamlit as stimport numpy as npimport sympy as spfrom scipy.optimize import minimizeimport plotly.graph_objects as goimport pandas as pdimport itertools
...
st.markdown("""<style>.main-header {background-color: #0F3050;padding: 10px 20px;border-radius: 8px;color: white;margin-bottom: 15px;}.main-header h1 { color: white; margin-bottom: 2px; font-size: 1.5rem;}.main-header h3 { color: #A0C0DF; margin-top: 0px; margin-bottom: 5px; font-size: 1rem; font-weight: normal;}.main-header p { color: #D0E0EF; font-size: 0.85rem; margin-bottom: 0px;}.stButton>button {background-color: #008C7A;color: white;font-weight: bold;border-radius: 5px;width: 100%;margin-top: 15px;}.stButton>button { background-color: #006B5D; color: white; }</style><div class="main-header"><h1>PROGRAMACIÓN NO LINEAL</h1><h3>Universidad de Cuenca</h3><p><b>Materia:</b> Investigación de Operaciones | <b>Carrera:</b> Ingeniería en Telecomunicaciones | <b>Autores:</b> Juan Pacheco, Jean Sumba</p></div>""", unsafe_allow_html=True)
...
def crear_funcion_1d(expr_str):x = sp.Symbol('x')expr = sp.sympify(expr_str)f_prime = sp.diff(expr, x)f_double_prime = sp.diff(f_prime, x)
...
def crear_funcion_nd(expr_str, n):simbolos = [sp.Symbol(f'x{i+1}') for i in range(n)] if n > 3 else sp.symbols('x1 x2 x3')[]expr = sp.sympify(expr_str)grad_expr = [sp.diff(expr, s) for s in simbolos]hess_expr = [[sp.diff(g, s2) for s2 in simbolos] for g in grad_expr]
...
def parse_vector(text):return np.array([float(x.strip()) for x in text.split(',')])

def parse_matrix(text):return np.array([[float(x.strip()) for x in line.split(',')] for line in text.strip().split('\n')])

def parse_restrictions(text):lines = text.strip().split('\n')A = []b = []for line in lines:if not line.strip(): continueparts = line.split(',')coeffs = [float(p.strip()) for p in parts[:-2]]val = float(parts[-2].strip())signo = parts[-1].strip()
...
def biseccion_1d(f_prime, a, b, tol, max_iter):hist = []c = aif f_prime(a) * f_prime(b) > 0:st.warning("El intervalo [a, b] no garantiza una raíz para f'(x) = 0 porque f'(a) y f'(b) tienen el mismo signo.")return c, hist
...
def newton_1d(f_prime, f_double_prime, x0, tol, max_iter):hist = []x = x0for i in range(max_iter):fp = f_prime(x)fdp = f_double_prime(x)hist.append({'Iteración': i+1, 'x': x, "f'(x)": fp, "f''(x)": fdp})if abs(fp) < tol:breakif fdp == 0:st.warning("La segunda derivada es cero, el método de Newton se detiene.")breakx = x - fp / fdpreturn x, hist

TEMA 2

def gradiente_nd(f, grad_f, x0, alpha, tol, max_iter, tipo="Minimizar"):hist = []x = np.array(x0, dtype=float)signo = -1 if tipo == "Minimizar" else 1for i in range(max_iter):g = np.array(grad_f(x))norm_g = np.linalg.norm(g)row = {'Iteración': i+1}for j, val in enumerate(x): row[f'x{j+1}'] = valrow['f(x)'] = f(x)row['||grad||'] = norm_ghist.append(row)

    if norm_g < tol: break
    x = x + signo * alpha * g
return x, hist

def newton_nd(f, grad_f, hess_f, x0, tol, max_iter, tipo="Minimizar"):hist = []x = np.array(x0, dtype=float)for i in range(max_iter):g = np.array(grad_f(x))norm_g = np.linalg.norm(g)row = {'Iteración': i+1}for j, val in enumerate(x): row[f'x{j+1}'] = valrow['f(x)'] = f(x)row['||grad||'] = norm_ghist.append(row)
...
def vertices_factibles(f, A, b_vec, tipo="Minimizar"):n = A.shape[1]m = A.shape[0]vertices = []
...
def proyectar(y, A, b_vec):res = minimize(lambda x: np.linalg.norm(x - y)**2, x0=y, constraints={'type': 'ineq', 'fun': lambda x: b_vec - A @ x})return res.x

def gradiente_proyectado(f, grad_f, A, b_vec, x0, alpha, tol, max_iter, tipo="Minimizar"):hist = []x = np.array(x0, dtype=float)x = proyectar(x, A, b_vec)signo = -1 if tipo == "Minimizar" else 1
...
def resolver_cuadratica(Q, c, const, A, b_vec, tipo="Minimizar"):def f_obj(x):val = 0.5 * x.T @ Q @ x + c.T @ x + constreturn val if tipo == "Minimizar" else -val
...
def graficar_1d(f, x_opt, a, b):xs = np.linspace(min(a, x_opt) - 2, max(b, x_opt) + 2, 400)ys = [f(xi) for xi in xs]fig = go.Figure()fig.add_trace(go.Scatter(x=xs, y=ys, mode='lines', name='f(x)', line=dict(color='#0F3050', width=2)))fig.add_trace(go.Scatter(x=[x_opt], y=[f(x_opt)], mode='markers', name='Óptimo', marker=dict(color='red', size=10)))fig.update_layout(title="Gráfica de la función", xaxis_title="x", yaxis_title="f(x)")return fig

def graficar_nd(f, n, x_opt, hist, A=None, b_vec=None):if n != 2:return None
...
st.sidebar.markdown("### TEMAS")opcion = st.sidebar.radio("Seleccione el método:", ["1. Una variable","2. Varias variables","3. Restringida linealmente","4. Optimización cuadrática"])

st.sidebar.markdown("---")st.sidebar.markdown("### Estado del programa")st.sidebar.info(f"Módulo Activo:\n\n{opcion}")

if opcion.startswith("1"):st.markdown("## 1. Optimización no restringida de una sola variable")col1, col2 = st.columns([1, 2], gap="large")
...
elif opcion.startswith("2"):st.markdown("## 2. Optimización no restringida de varias variables")col1, col2 = st.columns([1, 2], gap="large")
...
elif opcion.startswith("3"):st.markdown("## 3. Optimización restringida linealmente")col1, col2 = st.columns([1, 2], gap="large")
...
elif opcion.startswith("4"):st.markdown("## 4. Optimización cuadrática")col1, col2 = st.columns([1, 2], gap="large")
